...
int main() {
	...
	array<int, 5> arr; 
	arr.fill(10); -> /// {10, 10, 10, 10, 10} //all 10


	arr.at(index); //aceess index value

	for(int i = 0;i<5;i++) {
		cout << arr.at(i) << " "; //entire array print
	}


	// iterators 
	// begin()//(0th element), end()//(empty adreess right after last element), rbegin()//reverse begin(last), rend()//(reverse end,right before the start element) 
	...
	for(auto it = arr.rbegin()/*reverse iterartor*/; it>arr.rend();it++) {
		cout << *it << " "; //6 5 4 3 2 1 
	}
	...
	// for each loop 
	for(auto it: arr) {//auto is forward iterator
		cout << it << " "; 
	}
	...
	for(int i = 0;i<n;i++) {
		int x;
		cin >> x; 
		st.insert(x); //log(n) complexity n is size of set
	}
	/* Set Property:
	1) first element will be smallest element,2ND element will be seconfd smallest elemenet,lat element will be largest
    2)st[0] or st[1] or st[2] it is wrong
	3)we can access it as *st.begin() =first element , *st.begin()+1=second element ,*st.begin()+2= third elememnt
	*/
   ...
   //any order not guareente always ascending
	st.insert(2); 
	st.insert(3);
	st.insert(1); 
	...
	multiset<int> ms; 
    //store all element in sorted fashion even if it duplicate too
	ms.insert(1); 
	ms.insert(1); 
	ms.insert(2); 
	ms.insert(2); 
	...
	map<string, int> mpp; 
	//store in alphabetical dict order
	mpp["raj"] = 27; 
	mpp["hima"] = 31; 
	mpp["praveer"] = 31;
	mpp["sandeep"] = 67; 
	mpp["tank"] = 89; 
	mpp["raj"] = 29; 
	//raj store 29 because its updated
	mpp.emplace("raj", 45); 
	...
	// priority_queue //log(n)//sorted order
	// push 
	// size 
	// top pop empty 
...
//6 5 2 1//descending
   ...
   //1 2 5 6
    ...
    // question of pair 
    sort(arr, arr+n, greater<pair<int,int>>);//not work
